...
{
    #if defined(_WIN32_EXCEPT_NOT) //BEN_FLAG
        HMODULE hModule;
...
{
    #if defined(_WIN32)
        return LoadLibraryA(libraryPath); //BEN_FLAG (changed from LoadLibraryW)
    #else
                               ...
                               FilePathCharType* libraryPath, size_t libraryPathCapacity)
{
...

    {
        ...
        for(i = 0; i < sizeof(directoryArray)/sizeof(directoryArray[0]); ++i)
        {
                ...
                //BEN_FLAG (copied 3rd argument from WIN32 version of this line)
                ...
                printfResult = snprintf(libraryPath, libraryPathCapacity, "%lsLibOVRRT%hs_%d_%d.dll", directoryArray[i], pBitDepth, requestedProductVersion, requestedMajorVersion);
            #endif
